export const COMMODITIES = [
  ...
  { key: 'usd', label: 'Доллар США', enLabel: 'USD', ticker: null, short: 'USD', emoji: '💵' },
];
...
export async function getCommodityPrice(key) {
  if (key === 'usd') return 1; // USD — базовая валюта, её "цена" всегда 1
...
export async function getCommodityKlines(key, range) {
  if (key === 'usd') {
    // Плоская синтетическая история (всегда 1) — деление на неё просто возвращает цену второго актива как есть
